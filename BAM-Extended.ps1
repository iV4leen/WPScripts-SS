...
if (!(Test-Admin)) {
    Write-Warning "Ejecuta CMD como administrador."
    Start-Sleep 10
    Exit
}
...
Catch{
    Write-Warning "Error Parseando las BAM Key. Probablemente tu Windows no sea compatible para esto. "
    Exit
}
...
$Bam | Out-GridView -PassThru -Title "BAM Extended - Entradas BAM: $($Bam.count)  - Zona Horaria del Usuario: ($UserTime) -> ActiveBias: ( $Bias) - DayLightTime: ($Day) - rthy:)"
